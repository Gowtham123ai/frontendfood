    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-400 mt-1">Real-time store performance overview</p>
        </div>
        ...
        {cards.map((c, i) => (
          <Card key={i} className="relative overflow-hidden group">
            ...
            <div className="flex justify-between items-center relative z-10">
              <div>
                <p className="text-slate-400 text-sm font-medium mb-1">{c.title}</p>
         ...
         <Card className="md:col-span-2">
            ...
            <div className="space-y-4">
               {orders.slice(0, 5).map(o => (
                 <div key={o.id} className="flex items-center justify-between p-3 rounded-xl hover:bg-slate-700/50 transition-colors">
                    <div className="flex items-center gap-4">
                       <div className="w-10 h-10 rounded-full bg-slate-900 flex items-center justify-center text-slate-400 border border-slate-700 font-mono text-xs">
                         #{o.id.slice(-4)}
                       </div>
                       <div>
                          <p className="text-sm font-bold text-white">₹{o.totalAmount}</p>
                          <p className="text-[10px] text-slate-500">{getOrderDate(o).split('T')[0] || 'Recently'}</p>
                       </div>
                    </div>
         ...
         <Card title="System Performance">
            <div className="space-y-6 flex flex-col justify-center h-full pb-8">
               ...
               <div className="space-y-2">
                  ...
                  <div className="w-full h-1.5 bg-slate-700 rounded-full overflow-hidden">
                    <div className="w-[20%] h-full bg-orange-500" />
                  </div>
               </div>
            </div>
         </Card>
